...
class RescueRobot extends Robot implements Runnable {
    ...
    public RescueRobot(int robotId, int street, int avenue, Direction direction, int beepers, Color color,
            SharedGrid sharedGrid, int minI) {
    ...
    public void turnLeft() {
        ...
        if (nx >= 0 && nx < this.sharedGrid.getGrid().length && ny >= 0 && ny < this.sharedGrid.getGrid()[0].length
                && this.sharedGrid.getValue(nx, ny) == 5) {
            if (frontIsClear()) {
    ...
    public void rescuePeople() {
        synchronized (manager) {
            log("Rescuing people...");
            while (nextToABeeper() && this.rescuedPeople < MAX_PEOPLE) {
                pickBeeper();
    ...
    public ArrayList<Point> findPath(Point current, Point destination, boolean seek) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] direction : directions) {
                ...
                if (nx >= 0 && nx < this.sharedGrid.getLength() && ny >= 0 && ny < this.sharedGrid.getWidth()
                        && this.sharedGrid.getValue(nx, ny) != 0
                        && !visited.contains(new Point(nx, ny)) && nx2 >= 0 && nx2 < this.sharedGrid.getLength()
                        && ny2 >= 0
                        && ny2 < this.sharedGrid.getWidth() && !visited.contains(new Point(nx2, ny2))
                        && (!seek || (seek && this.sharedGrid.getValue(nx2, ny2) == 1))) {
    ...
    public Point moving(ArrayList<Point> path) {
        ...
        for (Point point : path) {
            ...
            if (answer != 1) {
                ...
                i++;
                if (nextToABeeper() && (this.getAvenue() - 1) * 2 >= 8 && (this.getAvenue() - 1) * 2 <= 28 && this.rescuedPeople<4) {
                    this.rescuingPeople();
                    return null;
                }
            }
        ...
        for (int i = this.minI; i <= this.maxI; i++) {
            for (int j = this.minJ; j <= this.maxJ; j++) {
                if (this.sharedGrid.isBeeper()) {
                    ...
                    if (beeper != null) {
                        this.sharedGrid.addCruce(this.getStreet(), this.getAvenue());
    ...
    public void waiting() {
        log("waiting");
        if (this.rescuedPeople >= 4) {
            this.rescuingPeople();
        }
        while (this.sharedGrid.getCompleted() < this.sharedGrid.getRobots()
        || (this.sharedGrid.isBeeper() || this.sharedGrid.isCruce())) {
            if (this.sharedGrid.isBeeper()) {
                Point beeper = this.sharedGrid.takeBeeper();
                if (beeper != null) {
                    log("Beeper taken "+beeper.getX()+", "+beeper.getY());
                    this.goTo(beeper.getX(), beeper.getY());
                }
            }
            if (this.sharedGrid.isCruce()) {
                ...
                if (cruce != null) {
                    ...
                    this.setMinI(this.getStreet());
                    this.setMinJ(this.getAvenue());
                    this.setMaxI(this.getStreet());
...
class Config {
    ...
    public static final String WORLD_FILE = "Testing.kwld";
